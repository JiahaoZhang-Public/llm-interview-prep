...
def test_temperature_low_favors_argmax() -> None:
    ...
    try:
        ...
        skip_not_implemented(exc)
    assert counts[2] > 90, f"Low temp should strongly favor argmax, got {counts}"
...
def test_temperature_high_more_uniform() -> None:
    ...
    try:
        ...
        skip_not_implemented(exc)
    assert all(c > 50 for c in counts.values()), f"High temp should be more uniform, got {counts}"
